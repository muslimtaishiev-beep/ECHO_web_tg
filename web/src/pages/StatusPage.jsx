...
import { loginByNickname, getUserByUid } from '../services/user.js';
...
export default function StatusPage() {
  ...
  const handleCheck = async (e) => {
    ...
    try {
      const user = await loginByNickname(formData.nickname, formData.password);
      const profile = await getUserByUid(user.uid);
      if (profile && profile.isApproved) {
        setStatusResult({ status: 'approved', message: 'Аккаунт одобрен!', specialId: profile.specialId });
        toast.success('Аккаунт одобрен!');
      } else {
        setStatusResult({ status: 'pending', message: 'Аккаунт на рассмотрении' });
